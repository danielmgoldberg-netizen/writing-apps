...
THE TASK
The student has read a text and written TWO sentences saying whether they liked or did not like the text, with reasons. They are following ONE of these templates:

TEMPLATE 1 (two reasons):
"I liked / didn't like this text for two main reasons. Firstly / First of all / To begin with, [reason 1]. Secondly / In addition / Also, [reason 2]."

TEMPLATE 2 (reason and example):
"I liked / didn't like this text because [reason]. For example / For instance, [example]."

You will be told:
1. The full text the student read.
2. The student's writing (which should be 2 sentences).
3. The attempt number (1, 2, 3, ...).

Detect which template the student is using and grade based on the closest match. Both opinions ("liked" and "didn't like") are equally valid. Never grade based on whether you agree with the opinion.

GRADING (out of 10 points total)
- Content (0 to 6 points): Are the reasons clear, specific to THIS text, and easy to understand?
- Sentence structure (0 to 2 points): Did they follow ONE template? Did they use suitable connectors (firstly, secondly, in addition, also, because, for example, for instance)?
- Language (0 to 2 points): Are vocabulary, word order, grammar, and punctuation correct? CHECK CAREFULLY:
  * Capitalization: Each sentence must start with a capital letter. The word "I" must always be capitalized. Proper nouns (names, places) must also be capitalized. Missing capitals lose points.
  * Punctuation: Each sentence must end with a period (.), question mark (?), or exclamation mark (!). BEFORE flagging a missing period at the end of a sentence, look at what character actually comes AFTER the last word. If the sentence already ends with ., ?, or !, then the punctuation is correct, so DO NOT flag it. Only flag missing punctuation if a sentence really ends without one.
...
CONTENT GUIDE (0-6)
- 6: The student's reasons (or reason + example) are clear enough to understand and relate to the text. Even a simple, short answer earns full marks. Depth or nuance is NOT required at this level.
- 4-5: The reasons are clear but only very general OR the connection to the text is loose.
- 2-3: The reasons are hard to follow OR barely connect to the text.
- 1: The reasons are very unclear or almost unrelated to the text.
- 0: The reasons do not connect to the text at all.

STRUCTURE GUIDE (0-2)
- 2: Two sentences, template followed, suitable connectors used.
- 1: Mostly there but with one issue (missing connector, only one sentence, mixed templates, etc.).
- 0: No template structure at all.

LANGUAGE GUIDE (0-2)
- 2: Grammar, spelling, capitalization, AND end-of-sentence punctuation are all mostly correct. Tiny slips are okay.
- 1: A few errors but the meaning is still clear. Missing a capital letter at the start of a sentence, missing a period at the end, or "i" not capitalized as "I" are issues that should drop a student to 1 even if everything else is correct.
- 0: Many errors that make it hard to understand.
...
- Small spelling slips do not always lose points. Use judgment.
- It is fine if the student says "didn't like" or "did not like" or "do not like".
- The student does not need to use the exact word from the template. Synonyms and small variations are fine if the meaning is the same.
- QUOTING FROM THE TEXT IS ALLOWED AND WELCOME. Students may borrow words, phrases, or short sentences directly from the article they read. Do NOT penalize this. Do NOT complain about "not using your own words". At A1-A2 level, quoting is a legitimate strategy for expressing ideas the student could not phrase independently. Only flag a plagiarism-style concern if the ENTIRE two-sentence response is copied verbatim with zero personal opinion added.
- HOWEVER, do NOT be lenient about: a missing capital letter at the start of a sentence, a missing period at the end of any sentence, "i" used instead of "I", or proper nouns not capitalized. These are basic mechanics that students must practice. Always flag them in the issues list.
- CONTENT IS ESPECIALLY LENIENT: In an exam context, students at this level have limited vocabulary and cannot always express nuance. If the reasons are understandable and connect to the text, award full content points (6/6). Do NOT deduct content points for "could have said more", "could go deeper", "reasons are simple", or "would benefit from more detail". You may add a suggestion for how to strengthen the writing, but that suggestion belongs in the issues list ONLY IF you also lost a point somewhere else. If content is 6/6, do not add a content issue at all.

FEEDBACK RULES (CRITICAL)
- For attempts 1, 2, and 3: NEVER write a fully correct sample. Set sampleSentence to null.
- Identify each error and EXPLAIN what is wrong, in very simple words.
- Give a HINT for how to fix it. Do NOT give the corrected sentence. For example, say "Your second reason is hard to understand. Try to use simpler words and explain WHY you felt that way." but never write the corrected sentence yourself.
...
ON ATTEMPT 4 OR LATER, if the student still has not reached 10/10:
- You MAY include ONE fully correct sample in sampleSentence to show them how it could look. Use whichever template fits their attempts best.
- Otherwise, sampleSentence is null.

If the student scores 10/10 at any attempt: sampleSentence is null and the encouragement should celebrate them.

LENGTH ENCOURAGEMENT (a nudge, never a deduction):
The goal is to grow students' fluency and analytical skills over time. A very short response like "I liked this text because I like Barbie. For example, she is famous." is technically complete but does not stretch the student. When you see a response like this:
- Award full points as long as the rubric is met (the student wrote clear, correct sentences that express an opinion).
- In your encouragement, add a warm nudge toward developing longer, more detailed responses next time. For example: "🌟 Well done! Next time, try adding more details about WHY you feel that way." or "✨ Great work! Can you tell me one more thing about what you liked?"
- Never phrase this as a criticism. Never deduct points for length. Never add a length issue to the issues list.
- A response is considered short if EITHER sentence is fewer than about 8-10 words, OR the total response is fewer than about 18-20 words.
- If the response is a reasonable length (roughly 20+ words total) and both sentences develop the point, just celebrate normally without a length nudge.

EMOJIS IN THE ENCOURAGEMENT
Add ONE friendly emoji at the START of your encouragement to make it warm.
- For 10/10, use a celebration emoji like 🎉, 🎊, ⭐, or ✨.
- For 7-9 out of 10, use a warm encouraging emoji like 👍, 💪, or 🌟.
- For 0-6 out of 10, use a kind, supportive emoji like 🌱, 💛, or 🤗.
...

{
  ...
  "totalScore": <sum of the three scores, 0 to 10>,
  "encouragement": "<one short positive sentence with one emoji at the start, max 30 words (may include a warm nudge toward writing more next time if the response was short)>",
  "issues": [
    {
      "category": "<content | structure | language>",
      "what": "<one short sentence saying what is wrong, max 25 words>",
      "hint": "<one short sentence telling them how to think about fixing it, max 30 words>"
    }
  ],
  "sampleSentence": <null or a string with one fully correct sample (two sentences using one template)>
}
...
const TEMPLATE_1_TEXT = "i liked / didn't like this text for two main reasons. firstly, ________. secondly, ________";
const TEMPLATE_2_TEXT = "i liked / didn't like this text because ________. for example, ________";
...
Grade this writing using the rubric. Detect which of the two templates the student is using. Remember the rules about sample sentences (never before attempt 4). Respond with JSON only.`;
}
...
function validateSubmission(text) {
  ...
  // 2. Do they still have BOTH 'liked /' and 'didn't like' present?
  // (Means they used the template but never chose which opinion is theirs.)
  // The regex handles straight, curly, or missing apostrophes.
  if (/liked\s*\/\s*didn[''‘’]?t\s*like/i.test(text)) {
    return "⚠️ Please choose ONE: did you LIKE the text, or did you NOT LIKE it? Delete the words you don't need.";
  }
...
// We only strip such complaints when the writing both ENDS in valid
// punctuation AND has at least one mid-text period (so both sentences
// look properly punctuated). This avoids overriding legitimate complaints
// about missing internal periods.
function sanitizeFeedback(feedback, submission) {
  ...
  // Count internal sentence-final marks (excluding the trailing one).
  const interior = trimmed.slice(0, -1);
  const hasInteriorPunct = /[.!?]/.test(interior);

  // Only override the AI when BOTH conditions hold: ends correctly AND
  // has at least one interior period. Anything less and the complaint
  // could be legitimate.
  if (!endsCorrectly || !hasInteriorPunct) return punctFixedFeedback;
  ...
  // Refund up to 1 language point per false positive removed, capped at the rubric max (2).
  const newScores = { ...punctFixedFeedback.scores };
  newScores.language = Math.min(2, newScores.language + removed);
  const newTotal = newScores.content + newScores.structure + newScores.language;
...
function ScoreRing({ score, max = 10 }) {
  ...
  const color = score === max ? "#2D5043" : score >= max * 0.6 ? "#7A6B2A" : "#B85C38";
  return (
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <header style={{ marginBottom: 24, paddingLeft: 6 }}>
          ...
          <p style={{
            ...
            Practice writing two sentences about whether you liked a text and why. Get friendly feedback, then try again.
          </p>
        </header>
        ...
        {selectedText && (
          <section style={cardStyle} className="mit-fade-in">
            <h2 style={stepTitleStyle}>
              ...
              ✍️ Write your opinion
            </h2>

            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: "#6B5D54", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10, fontWeight: 600 }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  I liked / didn't like this text for two main reasons. <span style={{ color: "#2D5043", fontStyle: "italic" }}>Firstly,</span> <span style={{ color: "#B85C38" }}>___</span>. <span style={{ color: "#2D5043", fontStyle: "italic" }}>Secondly,</span> <span style={{ color: "#B85C38" }}>___</span>.
                </div>
                <div style={{ fontSize: 12, color: "#6B5D54", marginTop: 8, lineHeight: 1.5 }}>
                  Other connectors you can use: <em>First of all, To begin with, In addition, Also.</em>
                </div>
                <div style={{ marginTop: 10, textAlign: "right" }}>
                  <button
                    className="mit-btn-template"
                    onClick={() => handleUseTemplate(TEMPLATE_1_TEXT)}
                  >
                    ↓ Use this template
                  </button>
                </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  I liked / didn't like this text <span style={{ color: "#2D5043", fontStyle: "italic" }}>because</span> <span style={{ color: "#B85C38" }}>___</span>. <span style={{ color: "#2D5043", fontStyle: "italic" }}>For example,</span> <span style={{ color: "#B85C38" }}>___</span>.
                </div>
                <div style={{ fontSize: 12, color: "#6B5D54", marginTop: 8, lineHeight: 1.5 }}>
                  Other connectors you can use: <em>For instance.</em>
                </div>
                <div style={{ marginTop: 10, textAlign: "right" }}>
                  <button
                    className="mit-btn-template"
                    onClick={() => handleUseTemplate(TEMPLATE_2_TEXT)}
                  >
                    ↓ Use this template
                  </button>
                </div>
            ...
            <div style={{
              ...
              ✋ The templates are in lowercase. <strong>You</strong> must add the capital letters (including "I" and the start of each sentence) and the period at the end of each sentence.
            </div>
            ...
            <textarea
              ...
              placeholder="✍️ Write your two sentences here..."
              disabled={isLoading}
            />
          ...
          >
            ...
            <div style={{ display: "flex", gap: 22, alignItems: "center", marginBottom: 18, flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <div style={{ fontSize: 13, color: "#6B5D54" }}>
                  Attempt {attempts.length} of unlimited.{sessionBest > 0 && ` Best so far: ${sessionBest}/10.`}
                </div>
              </div>
            </div>

            <div style={{ marginBottom: 18 }}>
              <CategoryBar label="Content" score={currentFeedback.scores.content} max={6} />
              <CategoryBar label="Sentence structure" score={currentFeedback.scores.structure} max={2} />
              <CategoryBar label="Language (grammar)" score={currentFeedback.scores.language} max={2} />
            </div>
            ...
            {currentFeedback.totalScore < 10 && (() => {
              ...
              let headline, body;
              if (score >= 8) {
                headline = "🌟 So close! Just a little more to fix.";
                body = "Read the hints above, then write again. You can reach 10 out of 10!";
              } else if (score >= 6) {
                headline = "💪 Good work! You are getting there.";
                body = "Look at the hints above and try again. Each attempt makes you better.";
              } else if (score >= 3) {
                headline = "🌱 Nice start! Now let's improve it together.";
                body = "Read each hint slowly. Then write your two sentences again, using the hints to help you.";
              } else {
                headline = "🤗 Don't worry, this is just practice!";
                body = "Read the hints above carefully, then try again. Every attempt helps you learn.";
              }
              return (
                <div style={{
                  padding: "14px 16px",
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{
                ...
                {currentFeedback.totalScore < 10
                  ? "✏️ Edit your writing, then try again"
